...
import qualified TodoBackend.Configuration     as Cfg
...
main = do
  ...
  config <- Cfg.getEnv' Cfg.serverConfig
  let url = Cfg.maybeServerUrl config "http://localhost"
  scotty (Cfg.maybeServerPort config 5000) $ do
