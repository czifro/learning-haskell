...
$(deriveJSON defaultOptions { fieldLabelModifier = drop 2 }
  ''TodoResponse)

mkTodoResponse :: String -> Sqlite.Entity Todo -> TodoResponse
mkTodoResponse rootUrl (Sqlite.Entity key Todo {..}) = TodoResponse
  key
  todoUrl
  todoTitle
  todoCompleted
  todoOrder
  where todoUrl = rootUrl ++ "/todos/" ++ Text.unpack (toPathPiece key)
...
actionToUpdates act = updateTitle ++ updateCompl ++ updateOrd
 where
  updateTitle = maybe [] (\title -> [TodoTitle Sqlite.=. title]) (actTitle act)
  updateCompl =
    maybe [] (\compl -> [TodoCompleted Sqlite.=. compl]) (actCompleted act)
  updateOrd = maybe [] (\ord -> [TodoOrder Sqlite.=. ord]) (actOrder act)

runDb :: Sqlite.SqlPersistT (ResourceT (NoLoggingT IO)) a -> IO a
runDb =
  runNoLoggingT
    . runResourceT
    . Sqlite.withSqliteConn "dev.sqlite3"
    . Sqlite.runSqlConn
